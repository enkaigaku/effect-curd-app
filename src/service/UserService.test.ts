...
describe("UserService", () => {
  it("should return paginated users", async () => {
    const mockUsers = [
      new User({ id: 1, name: "Alice", email: "alice@example.com", createdAt: new Date(), updatedAt: new Date() }),
      new User({ id: 2, name: "Bob", email: "bob@example.com", createdAt: new Date(), updatedAt: new Date() })
    ]
    const total = 10
    ...
    const MockRepo = Layer.succeed(
      UserRepository,
      UserRepository.of({
        findAll: (limit, offset) => Effect.succeed({ items: mockUsers, total }),
